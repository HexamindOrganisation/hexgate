 ...
 * (boundaries + that role's capabilities composed), not the open file. The
 * unsaved `draft` file is overlaid so the verdict matches the resolved preview.
 */
    ...
    <div className="h-full flex flex-col gap-3 p-4 overflow-y-auto scrollbar-thin">
      {disabled && (
        <p className="text-xs text-muted-foreground">
          {roleNames.length === 0
            ? "Declare a role (or a top-level grant) to test a call."
