...
async def test_when_a_recorder_is_erased_then_the_entry_survives_unattributed() -> None:
    """Erasing an operator must not be blocked by their classifications.

    A GDPR erasure request is a legal obligation with a deadline, so the
    delete goes through and the entry is left standing with no name on it.
    What that unattributed entry then means for the report is #246.

    Needs its own engine: SQLite ignores foreign keys — and therefore
    ``ON DELETE SET NULL`` — unless the pragma is set per connection.
    """
    ...
    try:
        ...
        async with factory() as s:
            after = await s.get(AgentClassification, entry_id)

        # The assertion survives; only the name it carried is gone.
        assert after is not None
        assert after.intended_purpose == "Asserted by them."
        assert after.recorded_by_user_id is None
    finally:
