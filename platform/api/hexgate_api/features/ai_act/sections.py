...
class AgentRecord:
    ...
    ``matrix`` is PR 2's Matrix or None, in which case ``matrix_unavailable``
...
def _matrix_rows(matrix: Any) -> list[dict[str, Any]]:
    """Tabulate PR 2's Matrix: one row per tool, one cell per role."""
    rows = []
    for tool in matrix.tools:
        cells = {}
        for role in matrix.roles:
            cell = matrix.cell(tool, role)
            cells[role] = {
                "mode": cell.mode,
                "label": MODE_LABELS.get(cell.mode, cell.mode),
                "constraint": cell.constraint_text,
            }
        rows.append({"tool": tool, "cells": cells})
    return rows
        ...
        agents.append(
            {
                ...
                "authorisation_matrix": (
                    {
                        "roles": list(record.matrix.roles),
                        "tools": list(record.matrix.tools),
                        "rows": _matrix_rows(record.matrix),
                    }
