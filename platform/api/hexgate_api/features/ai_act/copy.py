...
HUMAN_OVERSIGHT: Final = (
    ...
    "gating: a tool whose cell in this agent's authorisation matrix reads "
    "Approval is not "
    "executed on the caller's authority — the SDK returns a "
    "needs-approval decision to the host application, which must route the "
    "call to a person before it runs. Second, two operator kill switches: an "
...
DENY_BY_DEFAULT: Final = (
    "The matrix is complete for the tools named in the resolved bundle. A tool "
    "that a role's resolved policy does not mention is denied for that role: "
    "the cell reads Deny because nothing granted it, not because a rule "
    "refused it. Each cell is that one role's own mode. At call time a caller "
    "carrying several roles is resolved to the most permissive of their cells, "
    "which this per-role table does not show."
)
