...
def _matrix_for(
    ...
    *,
    ...
    if matrix_fn is None:
        return None, upstream.PENDING_MATRIX_REASON
    ...
    try:
        ...
        return matrix_fn(policy_set), None
...
async def _agent_records(session: AsyncSession, project_id: str) -> list[AgentRecord]:
    ...
    if modular:
        try:
            resolved = await modules.resolved_yaml_by_agent(
    ...
    matrix_fn = upstream.authorisation_matrix_fn()
