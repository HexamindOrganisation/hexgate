...
from hexgate.runtime.command_policy import Rejected, check_command
from hexgate.runtime.sandbox_runtime import build_sandbox_runtime_config
from hexgate.runtime.srt import ensure_srt_available
...
class LocalWorkspace(Workspace):
        ...
        *,
        ...
        if isinstance(policy, Rejected):
            ...
            return CommandResult(
                ...
                stdout="",
                stderr=(
                    f"hexgate: command not allowed: {policy.offending_token!r} "
                    f"({policy.reason})"
                ),
    ...
    def _write_sandbox_settings_file(self) -> str:
        ...
        fd, path = tempfile.mkstemp(suffix=".json", prefix="hexgate-srt-", text=True)
        try:
