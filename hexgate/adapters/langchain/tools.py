...
class SubagentTool(BaseTool):
    ...
    async def _arun(self, task: str, **_: Any) -> Any:
        ...
        if denial is not None:
            return denial
        # A child governance refusal is rendered as tool output (a completed delegation
        # that refused, not an abort), but it must not count as a *successful* execution.
        # So let it raise out of _delegate_child: in the pipeline path run_guarded_async
        # records it as an error (not ToolOutcome ok=True), and we render it here for both
        # paths — the same honesty the reach short-circuit above gives reach denials.
        try:
