...
text (``hexgate.security.rego``), Phase 3 turns that text into a
...
test against. Discovery checks ``$HEXGATE_OPA_BIN`` first, then falls
...
DEFAULT_ENTRYPOINTS: tuple[str, ...] = ("hexgate/policy/decision",)
...
class OpaNotFoundError(RuntimeError):
    """opa is not on PATH and ``$HEXGATE_OPA_BIN`` is unset or invalid."""
...
def compile_to_wasm(
    ...
    *,
    ...
    with tempfile.TemporaryDirectory(prefix="hexgate-opa-build-") as tmp:
        tmpdir = Path(tmp)
...
def _discover_opa() -> str:
    ...
    env = os.environ.get("HEXGATE_OPA_BIN")
    if env:
        if not Path(env).is_file():
            raise OpaNotFoundError(
                f"HEXGATE_OPA_BIN points to {env!r} but that file does not exist."
            )
        return env
    ...
    raise OpaNotFoundError(
        ...
        "or set HEXGATE_OPA_BIN to the binary path. "
