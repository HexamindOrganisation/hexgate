...
instruction under `callback_context.invocation_id`; `after_model_callback`
emits usage and messages together. The invocation id is the transcript key:
ADK rebuilds `contents` from the session on every call, so one invocation is
one message list, and an `AgentTool` runs a nested `Runner` that gets an id of
its own.

- **Streaming.** In SSE mode `after_model_callback` fires once per chunk, and
  each chunk repeats the call's running token counts. Only the non-partial
  aggregate ends a call and emits, so an answer is billed and recorded once.
  That aggregate carries no `model_version`, so the model name falls back to
  the stashed request's.
- **`transfer_to_agent` resyncs once.** A handoff stays on the same list, but
  ADK rewrites the other agent's turns into narrated text for the target, so
  the cursor sees a rewritten history and emits one `resynced` event per hop.
