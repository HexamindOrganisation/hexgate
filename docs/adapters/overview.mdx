...
**Google ADK**, or **Pydantic AI** to add three things without touching the
agent's logic:
...
3. **Audit and message logging.** Every wrapper emits the model's token counts
   and — unless `HEXGATE_LOG_MESSAGES=0` — the prompts and completions behind
   each decision: once per model call where the framework offers a hook for it,
   once per run on pydantic_ai, which does not. See [audit
   trail](/concepts/audit-trail#llm-messages).
...
| [Message logging](/concepts/audit-trail#llm-messages) | `on_llm_start` → `on_llm_end`, one event per model call | `on_chat_model_start` → `on_llm_end`, one event per model call | `before_model_callback` → `after_model_callback`, one event per model call | no per-call hook — one event per **run** |
