...
Nothing to wire. `HexgatePydanticAgent` records the conversation when a run
completes, alongside the token counts, and the audit drawer reads it back beside
the decisions of that run. See [audit
...
investigated. A session's rows concatenate back to the conversation either way.
