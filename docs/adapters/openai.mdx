...
output budget was sized for an answer, and the capper splits it evenly across
every string in the message, so enough reasoning blocks shrink the answer's
share along with their own. The input side has 256 KiB and no such squeeze,
which is why the two sides differ.
