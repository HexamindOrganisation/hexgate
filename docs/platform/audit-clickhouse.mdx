...
flowchart LR
  ...
  CH[("ClickHouse<br/>policy_decision · llm_invocation · ban_enforcement · llm_message")]:::hexBox
...
- **span-enricher** (`hexgate_api.jobs.enricher`, same image as the API): a
  ...
  LLM-usage, ban-enforcement or LLM-message event, resolves `agent_version_id`
  from Postgres, and batch-inserts into the four tables. Offsets commit only after
...
bypass the collector and are slated for removal. Don't build on them.
</Note>
