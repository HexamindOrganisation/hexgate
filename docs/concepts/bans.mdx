...
Every refused run reports a best-effort **blocked attempt** to the audit pipeline. The dashboard's **Blocked attempts** panel (on the Bans page) lists them by time, type, target, and reason, with a detail drawer per attempt. Blocked attempts are retained for 180 days.
