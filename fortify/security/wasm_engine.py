"""Evaluate a Fortify policy bundle's ``policy.wasm`` via wasmtime-py.
...
DEFAULT_ENTRYPOINT = "fortify/policy/decision"


class WasmPolicy:
    """A loaded, evaluable Fortify policy WASM bundle.
    ...
    def from_bundle_path(
        ...
        The bundle layout is the one ``fortify policy build`` emits:
    ...
    for arity_offset in range(5):
        ...
        def _stub(*args: int, _arity: int = arity_offset) -> int:
            ...
            raise WasmEvalError(
                ...
                f"(builtin{_arity}, id={builtin_id}); the Fortify "
