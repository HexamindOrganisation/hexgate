...
# ---------------------------------------------------------------------------
# build_signed_bundle — the producer shared by the CLI and the platform
# ---------------------------------------------------------------------------


@needs_opa
def test_build_signed_bundle_round_trips_through_from_parts() -> None:
    """The producer's output, fed straight into the consumer, verifies +
    decides — proving build (producer) and PolicyBundle (consumer) line up."""
    priv, pub = generate_keypair()
    sb = build_signed_bundle(
        _DEMO_YAML, source_name="billing.yaml", sign=lambda b: sign_bytes(b, priv)
    )
    assert sb.wasm_bytes is not None and sb.wasm_bytes.startswith(b"\x00asm")
    assert sb.manifest["source"] == "billing.yaml"
...
@needs_opa
def test_build_signed_bundle_no_wasm_no_sign() -> None:
    """compile_wasm=False + no signer → rego only, wasm_hash None, unsigned."""
    sb = build_signed_bundle(_DEMO_YAML, compile_wasm=False)
    assert sb.wasm_bytes is None
    assert sb.wasm_hash is None
    assert sb.signature is None
    assert sb.rego_text.startswith("# Generated by fortify policy build")


@needs_opa
def test_build_signed_bundle_is_byte_deterministic() -> None:
    """Same policy → identical manifest bytes (the seam's signing invariant)."""
    a = build_signed_bundle(_DEMO_YAML)
    b = build_signed_bundle(_DEMO_YAML)
    assert a.manifest_bytes == b.manifest_bytes


def test_build_signed_bundle_raises_on_bad_constraint() -> None:
    """A malformed constraint raises (callers translate to their own UX)."""
    bad = (
        "version: 1\nroles:\n  default:\n    tools:\n      t:\n        mode: allow\n"
        "        constraints: ['args.x ~~ 1']\n"
    )
